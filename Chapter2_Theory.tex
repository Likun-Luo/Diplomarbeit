...
We consider the linear wave equation (\ref{eq:wave_equation}) for one dimensional case with spatial coordinate $x$, which reads as 
\begin{equation}
...
\end{equation}
and the method will not be working. Therefore, we need to split up the linearized conservation equations (\ref{eq:linearised_convervation_mass}) and (\ref{eq:linearised_convervation_momentum}) into their spatial directions and introduce for each direction an artificial damping individually, which leads to the modified Helmholtz equation
\begin{equation}
...
\end{equation}
were introduced, with $\sigma_i$ being the damping functions in the spatial direction $i\in\lbrace x\text{,}y\text{,}z\rbrace$. The full derivation of (\ref{eq:modified_helmholtz_equation}) can be found in \cite{kaltenbacher_numerical_2007}. A second method to derive (\ref{eq:modified_helmholtz_equation}) is the so-called complex coordinate stretching, which maps the solution of Helmholtz equation in the real coordinate space to a complex coordinate space. This is done by defining the mapping e.g. for $x$
\begin{equation}
...
\end{equation}
The derivative (\ref{eq:derivative_stretched_coordinate}) can be transformed as
\begin{equation}
...
\end{equation}
Inserting the operator (\ref{eq:operator}) into the Helmholtz equation (\ref{eq:helmholtz_equation}) leads to the modified Helmholtz equation (\ref{eq:modified_helmholtz_equation}).
...
\end{equation}
where $p_\text{ref} = 20\,\mu\text{Pa}$ denotes the reference sound pressure, which is often considered as threshold of human hearing at 1000 Hz \cite{sinambari_ingenieurakustik_2020}. $p_{a\text{,rms}}$ denotes the root-mean-square value of the acoustic pressure and is also called the effective pressure, which is defined as the square root of the average of the square of the pressure of the sound signal over a given time duration $T$ and reads as
\begin{equation}
	p_{a\text{,rms}} = \sqrt{\frac{1}{T} \int_{t_0}^{t_0 + T} p_a^2(t)\,\text{d}t}\text{.}
\end{equation}
It can be shown that for a sinusoidal acoustic wave like (\ref{eq:sinusoidal_wave}), the effective pressure can be expressed as \cite{peterson_1972_handbook}
\begin{equation}
...
\end{equation}
with $\boldsymbol{n}$ being the surface normal vector. The sound power level is then defined as
\begin{equation}
...
\begin{figure}[H]
	...
	\end{subfigure}
	\caption{Example octave band and 1/3-octave band spectrum. Left: octave band spectrum. Right: 1/3-octave band spectrum}
